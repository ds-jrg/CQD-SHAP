...
def run(kbc_path, dataset_hard, dataset_complete, dataset_name, t_norm='min', candidates=3, scores_normalize=0, kg_path=None, explain=False):
	...
	experiments = [QuerDAG.TYPE1_2.value]
	print(experiments)
...
if __name__ == "__main__":
	...
	parser.add_argument('--explicit_path' , default=None,
					 help='Explicit path to the KG, if not provided, will use the path to the queries')
	...
	if args.explicit_path is not None:
		data_hard_path = osp.join(args.explicit_path)
		data_complete_path = osp.join(args.explicit_path)
	else:
